...
namespace CopticWriter.Views
{
    ...
    public sealed partial class DocumentEditorView : Page
    {
        ...
        private void DocumentEditorView_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            lastFocusedTextBox = (TextBox)docGrid.Children.First(c => c is TextBox);
            docGrid.Children.ToList().ForEach(element =>
            {
                if (element is TextBox box)
                {
                    box.TextChanged += lastFocusedTextBox_TextChanged;
                    box.GotFocus += (o, args) => lastFocusedTextBox = (TextBox)o;
                }
            });
        }
        ...
        public Doc Document
        {
            get => (Doc)GetValue(DocumentProperty);
            set => SetValue(DocumentProperty, value);
        }

        // Using a DependencyProperty as the backing store for Document.  This enables animation, styling, binding, etc...
        public static readonly DependencyProperty DocumentProperty =
            DependencyProperty.Register(nameof(Document), typeof(Doc), typeof(DocumentEditorView), new PropertyMetadata(null));
        ...
        {
            _characterCaret = lastFocusedTextBox.SelectionStart;
            lastFocusedTextBox.Text = lastFocusedTextBox.Text.Insert(lastFocusedTextBox.SelectionStart, ((Button)sender).Content.ToString());
            lastFocusedTextBox.SelectionStart = _characterCaret += 1;
        ...
        {
            _characterCaret = lastFocusedTextBox.SelectionStart;
            lastFocusedTextBox.Text = lastFocusedTextBox.Text.Insert(lastFocusedTextBox.SelectionStart, " ");
            lastFocusedTextBox.SelectionStart = _characterCaret += 1;
        }
        ...
        private void KeyBackspace_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                lastFocusedTextBox.SelectionStart = _characterCaret - 1;
                lastFocusedTextBox.Text = lastFocusedTextBox.Text.Remove(lastFocusedTextBox.SelectionStart);
                lastFocusedTextBox.SelectionStart = _characterCaret;

                if (lastFocusedTextBox.Text == string.Empty)
                {
                    lastFocusedTextBox.SelectionStart = 0;
                }
            }
        }
        ...
        private void lastFocusedTextBox_TextChanged(object sender, TextChangedEventArgs e)
        {
            int caret = lastFocusedTextBox.SelectionStart;
            switch ((Language)LanguageOption.SelectedIndex)
            {
                case CoptLib.Language.Coptic:
                    lastFocusedTextBox.Text = CopticInterpreter.ConvertFont(lastFocusedTextBox.Text, CopticFont.CsAvvaShenouda, CopticFont.CopticUnicode);
                    break;
            }
            lastFocusedTextBox.SelectionStart = caret;
        }
        #endregion
    }
}
