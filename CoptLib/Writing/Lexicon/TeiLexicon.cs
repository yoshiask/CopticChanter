...
public class TeiLexicon : ILexicon
{
        ...
        foreach (var elem in entryElem.Elements())
        {
            switch (elem.Name.LocalName)
            {
                case "form":
                    ...
                    var subGrammarGroupElem = elem.ElementLocal("gramGrp");
                    GrammarGroup? subGrammarGroup = subGrammarGroupElem is not null
                        ? ParseGrammarGroup(subGrammarGroupElem)
                        : null;

                    forms.Add(new Form(formType, usg, orth, subGrammarGroup));
                    break;

                case "gramGrp":
                    grammarGroup = ParseGrammarGroup(elem);
                    break;
    ...
    private static GrammarGroup ParseGrammarGroup(XElement elem)
    {
        ...
        return new(pos, num, gen, grammarEntries, subc, note);
    }
